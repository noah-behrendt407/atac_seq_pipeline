...
PEAKS_DIR="/"
OUTPUT_DIR=""
OUTPUT_NAME="Consensus_replicates_Shared"

mkdir -p "$OUTPUT_DIR"

# Identify the two replicates files
FILES=($PEAKS_DIR/*.bed)
FILE_A=${FILES[0]}
FILE_B=${FILES[1]}

echo "Processing Consensus for:"
echo "Sample A: $(basename $FILE_A)"
echo "Sample B: $(basename $FILE_B)"

# --- Step 1: Create the Union (The "Footprint") ---
# This merges both samples together so we have the widest possible coverage
cat "$FILE_A" "$FILE_B" | sort -k1,1 -k2,2n | bedtools merge -i stdin > ${OUTPUT_DIR}/temp_union.bed

# --- Step 2: Double Intersect (The "Filter") ---
# Only keep peaks from the Union that overlap BOTH Sample A and Sample B
# This ensures it isn't just a peak in one sample
...
echo "Sample A Count: $(wc -l < $FILE_A)"
echo "Sample B Count: $(wc -l < $FILE_B)"
